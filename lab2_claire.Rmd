...
```{r}
penguins_clean <- penguins %>% #commmand shift m for pipe function
  drop_na() %>% 
  rename(mass = body_mass_g, 
         bill_l = bill_length_mm,
         bill_d = bill_depth_mm,
         flip_l = flipper_length_mm)
#creating a model to predict mass
mdl1 <- lm(mass ~ bill_l + bill_d +flip_l + species + sex + island, 
           data = penguins_clean)
#summary to see our coeffients and summary of model  can usually just do this in console 
#summary(mdl1)
#we can also cheack out AIC 
#AIC(mdl1)
#4727.242
```
...
AIC(mdl1, mdl2)
#df = degrees of freedom (how many variables we have)

f3 <- mass ~ bill_d + flip_l + species + sex 

mdl3 <- lm(f3, data = penguins_clean)
#check out the AIC and BIC
AIC(mdl1, mdl2, mdl3)
...
set.seed(42) #this way we'll all get the same results 
...
train_df <-penguins_fold %>% 
  filter(group != 1)
#how to compare how well our model performs
```

```{r}
#Example function to find the mean of a vector 
...
rmse_predict_test <- predict_test %>% 
  summarise(rmse_mdl1 = calc_rmse(model1, mass), 
...
for(i in 1:folds){
  ...
  kfold_pred_df <- kfold_test_df %>% 
    mutate(mdl1 = predict(kfold_mdl1, kfold_test_df),
           ...
           mdl3 = predict(kfold_mdl3, .)) # the period tells the code to just run again kfold_test_df
